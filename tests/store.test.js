...
test("owner can create, edit, pause, resume, and delete an item", () => {
    ...
    assert.equal(store.getItem(item.id).pricePerDay, 90);
...
test("business quote, Pro upgrade, and Featured listing update simulated revenue", () => {
    store.resetDemo();
    const item = store.getItem("power-strip");
    const quote = store.calculateQuote(item.id, item.availableFrom, item.availableFrom);
    assert.equal(quote.days, 1);
    assert.equal(quote.rentalAmount, 30);
    assert.equal(quote.platformFee, 3);
    assert.equal(quote.ownerPayout, 27);
    assert.equal(quote.totalDue, 130);

    const before = store.getBusinessMetrics();
    store.login("niran@uniborrow.ac.th", "demo1234");
    store.upgradeToPro();
    assert.equal(store.getCurrentUser().plan, "pro");
    store.boostItem("power-strip");
    const after = store.getBusinessMetrics();
    assert.equal(after.subscriptionRevenue, before.subscriptionRevenue + 59);
    assert.equal(after.promotionRevenue, before.promotionRevenue + 19);
});
